...
export function validatePortalContract(l1ChainId: number, l2Chain: Chain) {
  const portal: ChainContract | undefined =
    l2Chain?.contracts?.portal?.[
      l1ChainId as keyof typeof l2Chain.contracts.portal
    ];

  if (!portal) {
    throw new Error(
      PortalContractNotConfiguredMessage(l1ChainId, l2Chain.name),
    );
  }
...
export function validateL2OutputOracleContract(
  l1ChainId: number,
  l2Chain: Chain,
) {
  const l2OutputOracle: ChainContract | undefined =
    l2Chain?.contracts?.l2OutputOracle?.[
      l1ChainId as keyof typeof l2Chain.contracts.portal
    ];
...
export function validatel1StandardBridgeContract(
  l1ChainId: number,
  l2Chain: Chain,
) {
  const l1StandardBridge: ChainContract | undefined =
    l2Chain?.contracts?.l1StandardBridge?.[
      l1ChainId as keyof typeof l2Chain.contracts.l1StandardBridge
    ];

  if (!l1StandardBridge) {
    // FIXME: Link to documentation for this
    throw new Error(l1StandardBridgeContractNotConfiguredMessage(l2Chain.name));
  }
...
export function validateL2StandardBridgeContract(l2Chain: Chain) {
  const l2StandardBridge = l2Chain?.contracts?.l2StandardBridge as
    | ChainContract
    | undefined;

  if (!l2StandardBridge) {
    throw new Error(l2StandardBridgeContractNotConfiguredMessage(l2Chain.name));
  }
